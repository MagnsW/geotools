...
class VesselSetup:
    ...
    def interactive_plot(self, gridspace, slider_inc, gridshift=True):
        ...
        def f(source_1, source_2, source_3, source_4, source_5, source_6):
            ...
            sns.scatterplot(x=sopos, y=1, marker='*', s=100, label='Sources')
            sns.scatterplot(x=recs, y=0.75, marker='v', s=100, label='Receivers')
            sns.scatterplot(x=midpt, y=0, label='Midpoints', alpha=0.5)
            cur_axes = plt.gca()
